...
class PaddleFLUXVaeDecoder:
    @classmethod
    def INPUT_TYPES(cls):
        return {"required": {"latent": ("LATENT",), "flux_pipe": ("PIPELINE",)}}
    ...
    def decode(self, flux_pipe, latent):
        vae = flux_pipe.vae
...
# class PaddleFLUXImage2ImagePipe:
#     @classmethod
#     def INPUT_TYPES(cls):
#         return {
#             "required": {
...
#     def sample(self, flux_pipe, image, denoise, prompt, negative_prompt, steps, number, seed, cfg, scheduler_type):
#         # torch -> numpy
#         if isinstance(image, torch.Tensor):
#             image = image.cpu().numpy()

#         pipe = StableDiffusionXLImg2ImgPipeline(**flux_pipe.components)
#         pipe.scheduler = get_scheduler(scheduler_type)
#         paddle.seed(seed)
...
#     def sample(self, flux_pipe, image, mask, denoise, prompt, negative_prompt, steps, number, seed, cfg, scheduler_type):
#         # torch -> numpy
#         if isinstance(image, torch.Tensor):
#             image = image.cpu().numpy()
...
#         pipe = StableDiffusionXLInpaintPipeline(**flux_pipe.components)
#         pipe.scheduler = get_scheduler(scheduler_type)
#         paddle.seed(seed)
...
NODE_CLASS_MAPPINGS = {
    ...
    "PaddleFLUXVaeDecoder": PaddleFLUXVaeDecoder,
    "PaddleFLUXText2ImagePipe": PaddleFLUXText2ImagePipe,
#     "PaddleFLUXImage2ImagePipe": PaddleFLUXImage2ImagePipe,
#     "PaddleFLUXInpaintPipe": PaddleFLUXInpaintPipe,
 }

NODE_DISPLAY_NAME_MAPPINGS = {
    "PaddleFLUXCheckpointLoader": "Paddle FLUX Checkpoint Loader",
    "PaddleFLUXVaeDecoder": "Paddle FLUX VAE Decoder",
    "PaddleFLUXText2ImagePipe": "Paddle FLUX Text2Image Pipe",
    # "PaddleFLUXImage2ImagePipe": "Paddle FLUX Image2Image Pipe",
    # "PaddleFLUXInpaintPipe": "Paddle FLUX Inpaint Pipe",
}
